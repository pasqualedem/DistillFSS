...
class Logger:
    ...
    def initialize(cls, args, training):
        ...
        cls.logpath = os.path.join(
            args.logpath,
            f"{'train' if training else 'test'}/fold_{args.fold}{logtime}"
        )
        ...
        log_file_path = os.path.join(cls.logpath, 'log.txt')
        print(f"Log file path: {log_file_path}")

        # Set up file handler
        file_handler = logging.FileHandler(log_file_path, mode='w')
        file_handler.setLevel(logging.INFO)
        file_formatter = logging.Formatter('%(message)s', datefmt='%m-%d %H:%M:%S')
