...
class DMTNetwork(nn.Module):
    ...
    def predict_mask_1shot(self, query_img, support_img, support_mask):
        ...
        return logit_mask, bg_logit_mask, pred_mask, self.hpn_learner.last_encoded
    ...
    def predict_mask_nshot(self, batch, nshot):
        ...
        for s_idx in range(nshot):
            logit_mask, bg_logit_mask, _, coarse_mask = self.predict_mask_1shot(batch['query_img'], batch['support_imgs'][:, s_idx], batch['support_masks'][:, s_idx])
            logit_mask_agg += logit_mask.argmax(dim=1)
            logit_mask_orig.append(logit_mask)
            bg_logit_mask_orig.append(bg_logit_mask)
            coarse_mask_orig.append(coarse_mask)
            if nshot == 1: return logit_mask_agg, logit_mask_orig, bg_logit_mask_orig, coarse_mask_orig
        ...
        return pred_mask, logit_mask_orig, bg_logit_mask_orig, coarse_mask_orig
